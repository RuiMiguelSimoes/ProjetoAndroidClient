                                ...
                                .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                                    ...
                                    public void onComplete(@NonNull Task<AuthResult> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            //inserir e atualizar utilizadores
                                            Map<String, Object> map = new HashMap<>();
                                            map.put("nome", nome);
                                            map.put("email", email);
                                            Intent intentNfc =new Intent();
                                            String existingUserID = intentNfc.getExtras().getString("userID");
                                           ;
                                            String newUserId = user.getUid();

                                            Map<String, Object> updadeCurrentUser = new HashMap<>();
                                            updadeCurrentUser.put("uid", existingUserID);
                                            updadeCurrentUser.put("nome", nome);
                                            updadeCurrentUser.put("email", email);

                                            Map<String, Object> newUser = new HashMap<>();
                                            newUser.put("uid", newUserId);
                                            newUser.put("nome", nome);
                                            newUser.put("email", email);

                                            databaseRef.child(existingUserID).child("amigos").push().setValue(updadeCurrentUser);
                                            databaseRef.child(user.getUid()).child("amigos").push().setValue(newUser);
